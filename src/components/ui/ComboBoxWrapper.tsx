...
export default function ComboBoxWrapper({
  ...
  return (
    <>
      <Combobox
        value={value}
        disabled={filteredOptions?.length < 1 ? true : false}
        onChange={onChange}
        nullable
      >
        ...
        <div className="relative">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <Combobox.Button className="w-full inset-y-0 right-0 flex items-center">
              <Combobox.Input
          ...
          >
            <Combobox.Options
              //   static={true}
              //   hold={true}
              className="absolute mt-1 z-40 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
            >
